...
class VisualPipelineBuilder extends EventEmitter {
      ...
      if (template) {
        Object.entries(template.config).forEach(([key, configDef]) => {
          if (configDef.required && !component.config[key]) {
            errors.push(
              `Component ${component.id} missing required config: ${key}`,
            );
          }
        });
      }
  ...
  generateJavaScriptCode(pipeline) {
    ...
    sortedComponents.forEach((component) => {
      ...
      if (template) {
        imports.add(
          `const ${template.name.replace(/\s+/g, "")} = require('@devilsdev/rag-pipeline-utils/src/plugins/${component.type}');`,
        );
        // eslint-disable-line global-require

        const configStr = JSON.stringify(component.config, null, 2);
        componentInits.push(
          `const ${component.id} = new ${template.name.replace(/\s+/g, "")}(${configStr});`,
        );
