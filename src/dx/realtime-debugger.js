...
class RealtimeDebugger extends EventEmitter {
  ...
  async executeComponent(componentId, input, context, step) {
    ...
    switch (componentId.split("_")[0]) {
      case "loader":
        output = {
          documents: [{ id: "1", content: "Sample document", metadata: {} }],
        };
        break;
      case "embedder":
      ...
      case "retriever":
        output = {
          results: [{ score: 0.95, document: { content: "Relevant content" } }],
        };
        break;
      case "llm":
  ...
  getExecutionTimeline(sessionId) {
    ...
    return {
      ...
      totalDuration: session.steps.reduce(
        (sum, step) => sum + (step.performance.duration || 0),
        0,
      ),
  ...
  handleClientMessage(clientId, data, ws) {
    switch (data._type) {
      case "addBreakpoint":
        try {
          const breakpointId = this.addBreakpoint(
            data.sessionId,
            data.componentId,
            data.condition,
          );
      ...
      case "continue":
        try {
          ...
          ws.send(
            JSON.stringify({ _type: "continued", sessionId: data.sessionId }),
          );
        } catch (error) {
          ws.send(JSON.stringify({ _type: "error", message: error.message }));
        }
      ...
      case "step":
        try {
          ...
          ws.send(
            JSON.stringify({ _type: "stepped", sessionId: data.sessionId }),
          );
        } catch (error) {
          ws.send(JSON.stringify({ _type: "error", message: error.message }));
        }
      ...
      default:
        ws.send(
          JSON.stringify({ _type: "error", message: "Unknown message _type" }),
        );
    }
  }
