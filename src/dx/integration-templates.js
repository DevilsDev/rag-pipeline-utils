...
/**
 * Normalize template shape and keep back-compat.
 * Google-style: small helpers, explicit transformation.
 */
function normalize(t) {
  return {
    ...
    type: t.type ?? t._type,
    config: t.config ?? t._config,
    generateIntegrationCode: t.generateIntegrationCode,
    category: t.category,
...
function addBackCompat(template) {
  if (!Object.getOwnPropertyDescriptor(template, "_type")) {
    Object.defineProperty(template, "_type", {
      get() {
        return template.type;
      },
    });
  }
  if (!Object.getOwnPropertyDescriptor(template, "_config")) {
    Object.defineProperty(template, "_config", {
      get() {
...
class IntegrationTemplates {
  ...
  initializeTemplates() {
    ...
    this.addTemplate("confluence", {
      ...
      config: {
        ...
        spaceKey: {
          _type: "string",
          required: false,
          description: "Specific space key (optional)",
        },
        pageLimit: {
          _type: "number",
          default: 100,
    ...
    this.categories.set("data-sources", {
      name: "Data Sources",
      description:
        "Connectors for various data sources and content management systems",
    });
  ...
  getTemplate(id) {
    ...
    return t ? addBackCompat(normalize(t)) : undefined;
  }
  ...
  getTemplatesByType(type) {
    return Array.from(this.templates.values())
      .filter((t) => (t.type ?? t._type) === type)
      .map((t) => addBackCompat(normalize(t)));
  }
