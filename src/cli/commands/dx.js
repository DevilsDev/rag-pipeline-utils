  ...
  builderCmd
    ...
    .action(async (_options) => {
      ...
      try {
        ...
        console.log(
          chalk.green("✅ Visual Pipeline Builder started successfully!"),
        ...
        // eslint-disable-line no-console
        console.error(
          chalk.red("❌ Failed to start Visual Pipeline Builder:"),
          error.message,
        ); // eslint-disable-line no-console
        process.exit(1);
      }
  ...
  builderCmd
    ...
    .action(async () => {
      ...
      pipelines.forEach((pipeline) => {
        ...
        console.log(
          `     Components: ${pipeline.components.length}, Connections: ${pipeline.connections.length}`,
        ); // eslint-disable-line no-console
...
function createDebuggerCommand() {
  ...
  debugCmd
    ...
    .action(async (sessionId, options) => {
      ...
      // eslint-disable-line no-console
      const session = await realtimeDebugger.startSession(
        sessionId,
        pipelineConfig,
      );
...
function createTemplatesCommand() {
  ...
  templatesCmd
    ...
    .action(async () => {
      ...
      console.log(
        chalk.cyan(
          `📊 ${stats.totalTemplates} templates available in ${stats.totalCategories} categories`,
        ),
      ); // eslint-disable-line no-console
      console.log(); // eslint-disable-line no-console
      const categories = templates.getAllCategories();
      categories.forEach((category) => {
        const categoryTemplates = templates.getTemplatesByCategory(
          category.name.toLowerCase().replace(/\s+/g, "-"),
        );
        console.log(
          chalk.yellow(`📁 ${category.name} (${categoryTemplates.length})`),
        ); // eslint-disable-line no-console
  ...
  templatesCmd
    ...
    .action(async (templateId, _options) => {
      ...
      if (_options.interactive) {
        ...
        const questions = Object.entries(template._config).map(
          ([key, configDef]) => ({
            ...
            validate: configDef.required
              ? (input) => (input ? true : "This field is required")
              : undefined,
            // eslint-disable-line no-console
          }),
        );
      ...
      try {
        ...
        console.log(
          chalk.green(`✅ Generated integration: ${integration.name}`),
        ); // eslint-disable-line no-console
        ...
        console.error(
          chalk.red("❌ Failed to generate integration:"),
          error.message,
        ); // eslint-disable-line no-console
      }
      // eslint-disable-line no-console
  ...
  dashboardCmd
    ...
    .action(async (__options) => {
      ...
      try {
        ...
        console.error(
          chalk.red("❌ Failed to start DX Dashboard:"),
          error.message,
        ); // eslint-disable-line no-console
        process.exit(1);
      }
