...
class AuditLogger extends EventEmitter {
  constructor(_options = {}) {
    ...
    this.logBuffer = [];
    this.encryptionKey =
      _options.encryptionKey || this._generateEncryptionKey();
    ...
    const auditEvent = {
      ...
      request: {
        ...
        parameters: event.parameters
          ? this._sanitizeParameters(event.parameters)
          : null,
      },
      result: event.result,
  ...
  _matchesQuery(log, query) {
    ...
    if (query.severity && log.metadata._severity !== query._severity)
      return false;
    if (
      query.correlationId &&
      log.metadata._correlationId !== query._correlationId
    )
      return false;
  ...
  async _sendToWebhook(logs) {
    ...
    try {
      ...
      logger.info(`Sending ${logs.length} audit logs to webhook`);
    } catch (error) {
