...
class DAG {
  ...
  async execute(seedOrOptions, _options = {}) {
    ...
    let timeoutId = null; // Declare at function scope
    ...
    if (
      ...
      (seedOrOptions.retryFailedNodes ||
        ...
        seedOrOptions.continueOnError ||
        seedOrOptions.requiredNodes ||
        seedOrOptions.checkpointId ||
        seedOrOptions.timeout ||
        seedOrOptions.maxConcurrency ||
        seedOrOptions.onProgress ||
        seedOrOptions.enableCheckpoints)
    ) {
      // First parameter is options object
      execOptions = seedOrOptions;
      seed = execOptions.seed || null;
    } else {
      // First parameter is seed, second is options
      seed = seedOrOptions;
    ...
    try {
      ...
      if (timeoutPromise) {
        await Promise.race([runAllNodes(), timeoutPromise]);
      } else {
        await runAllNodes();
      }
      ...
      // Check if we should throw based on critical failures
      const criticalErrors = [];
      for (const [nodeId, error] of errors.entries()) {
        const node = this.nodes.get(nodeId);
        const isRequired = requiredNodes.includes(nodeId);
        const hasDownstream = node && this.getDownstreamNodes(node).length > 0;

        if (isRequired || hasDownstream) {
          criticalErrors.push(error);
        }
      }

      // Throw if we have critical errors and not in graceful degradation mode
      if (criticalErrors.length > 0 && !gracefulDegradation) {
        if (criticalErrors.length === 1) {
          throw criticalErrors[0];
        } else {
          const aggregatedError = new Error('Multiple execution errors');
          aggregatedError.errors = criticalErrors;
      ...
      // Determine return value based on sink nodes
      const sinkNodes = Array.from(this.nodes.values()).filter(
        (node) => node.outputs.length === 0,
      );

      if (sinkNodes.length === 1) {
        // Single sink - return its result directly
      ...
      // Re-throw the error, preserving the original message
      throw error;
    }
  }
  ...
  async executeNode(node, context) {
    ...
    try {
      ...
      if (downstreamNodes.length > 0) {
        ...
        errorMsg = `Node ${node.id} execution failed: ${originalError.message}. This affects downstream nodes: ${downstreamIds}`;
      } else {
        errorMsg = `Node ${node.id} execution failed: ${originalError.message}`;
      }

      // Create node-scoped error with metadata
      const nodeError = new Error(errorMsg);
      nodeError.nodeId = node.id;
      nodeError.timestamp = Date.now();
      nodeError.cause = originalError;
      ...
      if (
        ...
        errors.set(node.id, nodeError);
        // Do NOT set results.set(node.id, null) for optional/non-critical nodes
        return null;
      }

      // For critical failures, store and throw the node error
      errors.set(node.id, nodeError);
      throw nodeError;
    }
  }
  ...
  async resume(checkpointData, seed = null) {
    ...
    if (checkpointData instanceof Map) {
      for (const [nodeId, result] of checkpointData) {
        results.set(nodeId, result);
      }
    } else if (checkpointData && typeof checkpointData === 'object') {
      for (const [nodeId, result] of Object.entries(checkpointData)) {
        results.set(nodeId, result);
      }
    }
