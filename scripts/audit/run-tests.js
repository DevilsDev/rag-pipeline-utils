...
class TestRunner {
  ...
  generateQualityReport() {
    ...
    const suitePassRate =
      this.results.summary.suites.total > 0
        ? (
            (this.results.summary.suites.passed /
              this.results.summary.suites.total) *
            100
          ).toFixed(1)
        : "0";
    ...
    .then((results) => {
      const passRate =
        results.summary.totalTests > 0
          ...
          : "0";
      console.log(
        `🎯 Test Results: ${results.summary.passedTests}/${results.summary.totalTests} (${passRate}%)`,
      );
