...
class SecurityAuditor {
  ...
  async runNpmAudit() {
    ...
    return new Promise((resolve) => {
      ...
      auditProcess.on('close', (code) => {
        ...
        console.log(`📊 npm audit: ${this.results.npmAudit.totalVulnerabilities} vulnerabilities found`);
        resolve();
      });
  ...
  parseNpmAuditResults(auditData) {
    ...
    // Calculate total
    this.results.npmAudit.totalVulnerabilities = Object.values(this.results.npmAudit.vulnerabilities)
  ...
  async analyzeSupplyChain() {
    ...
    try {
      ...
      if (fs.existsSync(lockPath)) {
        console.log('✅ package-lock.json found - dependency versions are locked');
      } else {
        this.results.supplyChain.unpinnedDependencies.push({
          package: 'lockfile',
          version: 'missing',
  ...
  async scanForSecrets() {
    ...
    const secretPatterns = [
      { name: 'API Key', pattern: /[aA][pP][iI][_\s]*[kK][eE][yY][_\s]*[:=]\s*['"]?[a-zA-Z0-9]{20,}['"]?/ },
      { name: 'AWS Access Key', pattern: /AKIA[0-9A-Z]{16}/ },
      { name: 'Private Key', pattern: /-----BEGIN [A-Z ]*PRIVATE KEY-----/ },
      { name: 'JWT Token', pattern: /eyJ[a-zA-Z0-9_-]*\.eyJ[a-zA-Z0-9_-]*\.[a-zA-Z0-9_-]*/ },
      { name: 'Password', pattern: /[pP][aA][sS][sS][wW][oO][rR][dD][_\s]*[:=]\s*['"]?[a-zA-Z0-9!@#$%^&*]{8,}['"]?/ },
      { name: 'Database URL', pattern: /[a-zA-Z][a-zA-Z0-9+.-]*:\/\/[a-zA-Z0-9._-]+:[a-zA-Z0-9._-]+@[a-zA-Z0-9.-]+/ }
    ];
    ...
    console.log(`🔍 Found ${this.results.secretsHygiene.potentialSecrets.length} potential secrets`);
  }
  ...
  findFilesToScan() {
    const files = [];
    const extensions = ['.js', '.ts', '.json', '.env', '.yml', '.yaml', '.md', '.txt'];
  ...
  calculateOverallRisk() {
    ...
    // Secrets
    const highSecrets = this.results.secretsHygiene.potentialSecrets.filter(s => s.severity === 'high').length;
    ...
    // Docker issues
    const dockerHighIssues = this.results.dockerSecurity.dockerfiles
      .reduce((sum, df) => sum + df.issues.filter(i => i.severity === 'high').length, 0);
...
${this.results.npmAudit.advisories.length > 0 ? `### Top Vulnerabilities

${this.results.npmAudit.advisories.slice(0, 5).map(advisory => 
...
${this.results.supplyChain.unpinnedDependencies.length > 0 ? `### Unpinned Dependencies

${this.results.supplyChain.unpinnedDependencies.slice(0, 10).map(dep => 
  `- **${dep.package}**: ${dep.version} (${dep.risk})`
).join('\n')}
...
${this.results.secretsHygiene.potentialSecrets.length > 0 ? `### Detected Secrets

${this.results.secretsHygiene.potentialSecrets.map(secret => 
...
${this.results.dockerSecurity.dockerfiles.map(dockerfile => `### ${dockerfile.file}

${dockerfile.issues.length > 0 ? `**Issues:**
${dockerfile.issues.map(issue => `- **${issue.severity.toUpperCase()}:** ${issue.message}`).join('\n')}
` : '✅ No security issues found'}

${dockerfile.recommendations.length > 0 ? `**Recommendations:**
...
`;
  ...
  generateRecommendations() {
    ...
    if (this.results.npmAudit.vulnerabilities.critical > 0) {
      recommendations.push('**URGENT:** Fix critical npm vulnerabilities using `npm audit fix`');
    }
    
    if (this.results.secretsHygiene.potentialSecrets.filter(s => s.severity === 'high').length > 0) {
      recommendations.push('**URGENT:** Remove or secure high-severity secrets detected in codebase');
    }
    
    if (!this.results.securityFiles.securityPolicy) {
      recommendations.push('Create SECURITY.md file with vulnerability reporting process');
    }
    
    if (this.results.supplyChain.unpinnedDependencies.length > 5) {
      recommendations.push('Pin dependency versions and maintain package-lock.json');
    }
  ...
  async executeSecurityAuditWithReports() {
    ...
    fs.writeFileSync(
      path.join(docsDir, 'SECURITY_AUDIT.md'),
      securityReport
    );
...
if (require.main === module) {
  ...
  auditor.executeSecurityAuditWithReports()
    .then(results => {
      console.log(`🎯 Security Risk: ${results.overallRisk.toUpperCase()}`);
      console.log(`🎯 Vulnerabilities: ${results.npmAudit.totalVulnerabilities}`);
      console.log(`🎯 Potential Secrets: ${results.secretsHygiene.potentialSecrets.length}`);
      process.exit(0);
    })
    .catch(error => {
