...
class RepositoryInventory {
  ...
  summarizeHeavyDirectory(dirPath) {
    ...
    try {
      ...
      console.warn(
        `Could not summarize heavy directory ${dirPath}: ${error.message}`,
      );
    }
  ...
  isTestFile(filePath, content) {
    const testPatterns = [/__tests__/, /\.test\./, /\.spec\./, /test/i];
  ...
  quickESLintCheck(content) {
    ...
    const undefPatterns = [/\b\w+\s+is not defined/g, /ReferenceError/g];
  ...
  async generateInventory() {
    ...
    // Write JSON files
    fs.writeFileSync(
      path.join(reportsDir, "full-inventory.json"),
      JSON.stringify(this.files, null, 2),
    );
    fs.writeFileSync(
      path.join(reportsDir, "folder-inventory.json"),
      JSON.stringify(this.folders, null, 2),
    );

    console.log(
      `✅ Inventory complete: ${this.files.length} files, ${this.folders.length} folders`,
    );
    console.log(
      "📁 Generated: ci-reports/full-inventory.csv, full-inventory.json",
    );
    console.log(
      "📁 Generated: ci-reports/folder-inventory.csv, folder-inventory.json",
    );
