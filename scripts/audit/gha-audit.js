...
class GitHubActionsAuditor {
  ...
  async auditWorkflows() {
    ...
    for (const workflowFile of workflowFiles) {
      try {
        ...
        console.warn(`Warning: Could not audit ${workflowFile}: ${error.message}`);
      }
    }
    
    console.log(`✅ Audited ${this.results.metadata.auditedWorkflows} workflows`);
    return this.results;
  }
  ...
  auditWorkflowStructure(workflow, audit) {
    ...
    if (workflow.concurrency) {
      ...
      audit.warnings.push({
        type: 'concurrency',
        message: 'No concurrency control defined - may cause resource conflicts'
      });
    }
    ...
    if (workflow.permissions) {
      ...
      if (workflow.permissions === 'write-all' || 
        ...
        audit.securityIssues.push({
          type: 'permissions',
          severity: 'high',
          message: 'Overly broad permissions detected - use minimal required permissions'
        });
      }
    } else {
      audit.securityIssues.push({
  ...
  looksLikeSecret(key, value) {
    const secretKeywords = ['token', 'key', 'secret', 'password', 'pass', 'auth'];
...
${this.results.workflows.map(workflow => `### ${workflow.displayName} (${workflow.name})

**Status:** ${workflow.status === 'passed' ? '✅ PASSED' : workflow.status === 'warning' ? '⚠️ WARNING' : '❌ FAILED'}

${workflow.securityIssues.length > 0 ? `**Security Issues (${workflow.securityIssues.length}):**
${workflow.securityIssues.map(issue => `- **${issue.severity.toUpperCase()}:** ${issue.message}`).join('\n')}
` : ''}

${workflow.issues.length > 0 ? `**Issues (${workflow.issues.length}):**
${workflow.issues.map(issue => `- ${issue.message}`).join('\n')}
` : ''}

${workflow.warnings.length > 0 ? `**Warnings (${workflow.warnings.length}):**
...
${this.results.patches.length > 0 ? `${this.results.patches.length} patch files generated in \`ci-reports/gha-patches/\`:
...
`;
  ...
  async executeGHAAudit() {
    ...
    if (this.results.patches.length > 0) {
      console.log(`📁 Generated: ${this.results.patches.length} patch files in ci-reports/gha-patches/`);
    }
...
if (require.main === module) {
  ...
  auditor.executeGHAAudit()
    .then(results => {
      console.log(`🎯 GHA Audit: ${results.summary.passed} passed, ${results.summary.failed} failed`);
