...
let eslintOutput;
const result = sh("npm", ["run", "lint:errors-only", "--", "--format=json"]);
if (result.success) {
  console.log("✅ No ESLint errors found!"); // eslint-disable-line no-console
  process.exit(0);
} else {
  eslintOutput = result.stdout;
}
...
try {
  ...
  const terminalErrors = [
    ...
    },
    {
      ...
      vars: ["fs", "path", "tenantId", "request", "data", "context"],
    },
    {
      file: "src/enterprise/multi-tenancy.js",
      vars: ["tenantId", "workspaceId"],
    },
    {
