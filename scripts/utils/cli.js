 ...
 */
function showHelp(_scriptName, _description, options = {}) {
 ...
 */
async function dryRunWrapper(_isDryRun, _operation, _fn) {
  if (_isDryRun) {
    loggerModule.info(`[DRY-RUN] ${_operation}`);
    return { dryRun: true, operation: _operation };
  }

  loggerModule.info(_operation);
  return await _fn();
}
 ...
 */
function validateArgs(_args, required = [], scriptName = "script") {
  const missing = [];
  ...
  if (missing.length > 0) {
    loggerModule.error(`Missing required arguments: ${missing.join(", ")}`);
    process.exit(1);
  }
}
 ...
 */
function setupCLI(_scriptName, _description, options = {}) {
  ...
  const scriptLogger = loggerModule;
...
module.exports = { parseArgs, showHelp, dryRunWrapper, validateArgs, setupCLI };
