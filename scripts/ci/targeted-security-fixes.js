        ...
        for (const [stepIndex, step] of job.steps.entries()) {
          if (step.run && typeof step.run === "string") {
            ...
            const hasErrorHandling =
              ...
              step.run.includes("set -o pipefail");
            if (!hasErrorHandling && step.run.length > 20) {
              step.run = "set -e\nset -o pipefail\n" + step.run;
              fixed = true;
              console.log(
                `  🛡️ Added shell hardening to job '${jobName}' step ${stepIndex}`,
              );
            }
          }
        }
...
async function main() {
  ...
  try {
    ...
    const failingWorkflows = auditData.workflows.filter(
      (w) => w.issues && w.issues.length > 0,
    );

    console.log(
      `📊 Found ${failingWorkflows.length} workflows with security issues`,
    );
