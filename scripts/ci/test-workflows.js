...
function testSecurityCompliance(workflowPath) {
      ...
      if (job.steps) {
        for (const [stepIndex, step] of job.steps.entries()) {
          if (step.run && typeof step.run === 'string') {
            if (!step.shell) {
              shellIssues++;
              issues.push(`Job '${jobName}' step ${stepIndex} missing explicit shell`);
            }
            
            const lines = step.run.split('\n');
            if (lines.length > 1 && !step.run.includes('set -e')) {
              shellIssues++;
              issues.push(`Job '${jobName}' step ${stepIndex} missing error handling`);
            }
          }
        }
      }
  ...
  return { issues, passed, score: passed.length / (passed.length + issues.length) * 100 };
}
...
function generateTestReport(results) {
  ...
  for (const result of results) {
    const status = result.overall.status === 'PASS' ? '✅' : 
                  result.overall.status === 'WARN' ? '⚠️' : '❌';
    const securityScore = `${result.security.score.toFixed(1)}%`;
    const structureScore = `${result.structure.score.toFixed(1)}%`;
    const issueCount = result.security.issues.length + result.structure.issues.length;
    
    markdown.push(`| ${result.name} | ${status} | ${securityScore} | ${structureScore} | ${issueCount} |`);
  }
  ...
  for (const result of results) {
    ...
    markdown.push(`**YAML Syntax:** ${result.yaml.valid ? '✅ Valid' : '❌ Invalid'}`);
    markdown.push('');
    ...
    if (result.structure.issues.length > 0) {
      ...
      result.structure.issues.forEach(issue => markdown.push(`- ❌ ${issue}`));
      markdown.push('');
    }
...
async function main() {
  ...
  for (const workflow of workflows) {
    ...
    if (!yamlTest.valid || securityTest.score < 70 || structureTest.score < 80) {
      overallStatus = 'FAIL';
  ...
  const successRate = (report.summary.passed / report.summary.totalWorkflows * 100).toFixed(1);
