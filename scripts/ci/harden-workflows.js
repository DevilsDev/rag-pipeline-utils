...
function yamlLintAll() {
  ...
  try {
    ...
    return {
      ok: false,
      error: `YAML parse validation failed: ${error.message}`,
    };
  }
}
...
function applyPatchFile(patchPath) {
  ...
  try {
    ...
    if (!patchData.workflow || !patchData.patches) {
      return {
        ok: false,
        error: "Invalid patch format - missing workflow or patches",
      };
    }
    ...
    if (!fs.existsSync(workflowFile)) {
      return {
        ok: false,
        error: `Workflow file not found: ${patchData.workflow}`,
      };
    }
      ...
      switch (patch.type) {
        ...
        case "add_timeout":
          if (
            ...
            modifiedWorkflow.jobs[patch.job]
          ) {
            if (!modifiedWorkflow.jobs[patch.job]["timeout-minutes"]) {
              modifiedWorkflow.jobs[patch.job]["timeout-minutes"] =
                patch.changes["timeout-minutes"];
            }
          }
          break;
    ...
    const modifiedContent = yaml.dump(modifiedWorkflow, {
      indent: 2,
      lineWidth: 120,
    });
...
function revertChangedWorkflows() {
  try {
    const changed = sh("git diff --name-only -- .github/workflows", {
      cwd: ROOT,
    })
      .trim()
      .split("\n")
...
async function main() {
  ...
  if (patches.length === 0) {
    console.log(
      "No patches found in ci-reports/gha-patches. Nothing to apply.",
    );
  ...
  try {
    ...
    console.error(
      "js-yaml package required but not found. Please install: npm install js-yaml",
    );
    process.exit(1);
  }
  ...
  if (fs.existsSync(AUDIT_MD)) {
    fs.copyFileSync(
      AUDIT_MD,
      path.join(OUT_DIR, "GITHUB_ACTIONS_AUDIT_BEFORE.md"),
    );
  }
  ...
  console.log(
    `📈 Baseline: ${beforeSummary.passed}/${beforeSummary.total} workflows passing, ${beforeSummary.securityIssues} security issues`,
  );
  ...
  for (const patchPath of patches) {
    ...
    if (!applyRes.ok) {
      ...
      writeArtifact(
        `patch-${patchName}-FAILED.txt`,
        applyRes.error || "apply failed",
      );
    ...
    if (!yamlRes.ok) {
      ...
      writeArtifact(
        `patch-${patchName}-ROLLED_BACK.yaml-lint.txt`,
        yamlRes.error || "yaml lint failed",
      );
    ...
    try {
      ...
      sh(
        `git commit -m "chore(ci): apply hardening patch ${patchName}" --quiet`,
      );
      results.push({
        patch: patchName,
        status: "APPLIED",
  ...
  if (fs.existsSync(AUDIT_MD)) {
    fs.copyFileSync(
      AUDIT_MD,
      path.join(OUT_DIR, "GITHUB_ACTIONS_AUDIT_AFTER.md"),
    );
  }
  ...
  console.log(
    `📈 After patches: ${afterSummary.passed}/${afterSummary.total} workflows passing, ${afterSummary.securityIssues} security issues`,
  );

  // 4) Check if we achieved 100% success
  const stillFailing =
    (afterSummary.failed ?? 0) > 0 || (afterSummary.securityIssues ?? 0) > 0;
  ...
  if (stillFailing) {
    ...
    const changed = sh(
      'git log --oneline --name-only --since="1 hour ago" -- .github/workflows',
      { cwd: ROOT },
    )
      .trim()
      .split("\n")
    ...
    if (rolledBack.length > 0) {
      try {
        sh("git add .github/workflows");
        sh(
          'git commit -m "revert(ci): rollback problematic workflow changes (auto)" --quiet',
        );
  ...
  console.log(
    `📈 Improvement: ${beforeSummary.securityIssues} → ${afterSummary.securityIssues} security issues`,
  );
