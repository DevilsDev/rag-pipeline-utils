          ...
          if (step.run && typeof step.run === "string") {
            ...
            if (lines.length > 1) {
              const hasShellSafety =
                newRun.includes("set -e") && newRun.includes("set -o pipefail");
              if (!hasShellSafety) {
                const safetyHeaders = ["set -e", "set -o pipefail", "set -u"];
            ...
            if (stepFixed) {
              fixed++;
              console.log(
                `    🛡️ Hardened shell security in job '${jobName}' step ${stepIndex}`,
              );
            }
          }
        }
        ...
        for (const [stepIndex, step] of job.steps.entries()) {
          if (step.env) {
            ...
            if (envFixed) {
              fixed++;
              console.log(
                `    🔒 Secured environment variables in job '${jobName}' step ${stepIndex}`,
              );
            }
          }
        }
        ...
        for (const [stepIndex, step] of job.steps.entries()) {
          if (step.run && typeof step.run === "string") {
            ...
            if (step.run.includes("$") && !step.run.includes('"')) {
              issues.push(
                `Job '${jobName}' step ${stepIndex}: Unquoted variables detected`,
              );
            }

            if (step.run.includes("eval") || step.run.includes("exec")) {
              issues.push(
                `Job '${jobName}' step ${stepIndex}: Dangerous commands (eval/exec) detected`,
              );
            }
            ...
            if (
              ...
              issues.push(
                `Job '${jobName}' step ${stepIndex}: Multi-line script missing error handling`,
              );
            }
          }
        }
