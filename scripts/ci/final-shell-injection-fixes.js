      ...
      if (job.steps) {
        for (const [stepIndex, step] of job.steps.entries()) {
          ...
          if (step.run && typeof step.run === 'string') {
            ...
            newRun = newRun.replace(/echo\s+"([^"]*)"(\$[A-Z_][A-Z0-9_]*)""/g, 'echo "$1$2"');
            
            // 2. Fix node command patterns with variables
            // Replace: node script.js v"$VERSION"
            newRun = newRun.replace(/node\s+([^\s]+)\s+v"(\$[A-Z_][A-Z0-9_]*)"/g, 'node $1 "v$2"');
            
            // 3. Fix VERSION variable patterns
            // Replace: VERSION="${GITHUB_REF#refs/tags/v}"
            newRun = newRun.replace(/VERSION="\$\{([^}]+)\}"/g, 'VERSION="${$1}"');
            ...
            newRun = newRun.replace(/\$\(([^)]+)\$([A-Z_][A-Z0-9_]*)\)/g, '$(${1}${$2})');
            
            // 6. Ensure all variables in strings are properly enclosed
            newRun = newRun.replace(/(\$[A-Z_][A-Z0-9_]*)/g, (match, variable) => {
              // Don't modify if already in quotes or braces
              if (newRun.indexOf('"' + variable + '"') !== -1 || 
            ...
            if (lines.length > 1) {
              ...
              if (!hasSetE || !hasSetPipefail) {
                ...
                const firstSetIndex = lines.findIndex(line => line.trim().startsWith('set '));
                if (firstSetIndex >= 0) {
          ...
          if (stepFixed) {
            fixed++;
            console.log(`    🔧 Fixed shell injection patterns in job '${jobName}' step ${stepIndex}`);
          }
        }
      }
    }
        ...
        for (const [stepIndex, step] of job.steps.entries()) {
          if (step.run && typeof step.run === 'string') {
            ...
            if (/echo\s+[^"'].*\$[A-Z_]/.test(step.run)) {
              issues.push(`${jobName}[${stepIndex}]: Unquoted variables in echo`);
            }
            
            // Mixed quoting patterns
            if (/"[^"]*"\$[A-Z_][A-Z0-9_]*"/.test(step.run)) {
              issues.push(`${jobName}[${stepIndex}]: Mixed quoting pattern detected`);
            }
            
            // Command injection risks
            if (/\$\([^)]*\$[A-Z_]/.test(step.run)) {
              issues.push(`${jobName}[${stepIndex}]: Potential command injection`);
            }
            
            // Missing error handling in multi-line scripts
            if (step.run.split('\n').length > 2 && !step.run.includes('set -e')) {
...
function fixWorkflowShellInjection(filePath) {
  ...
  if (fixesApplied > 0) {
    ...
    console.log(`  ✅ Applied ${fixesApplied} shell injection fixes to ${name}`);
    console.log(`  📊 Security issues: ${beforeIssues.length} → ${afterIssues.length}`);
...
async function main() {
  ...
  try {
    ...
    console.log(`📊 Total security issues: ${totalIssuesBefore} → ${totalIssuesAfter}`);
    console.log(`🔒 Improvement: ${((totalIssuesBefore - totalIssuesAfter) / totalIssuesBefore * 100).toFixed(1)}% reduction`);
