...
const TARGETED_FIXES = [
  {
    ...
    action: () => {
      ...
      const e2eTestPath = path.join(process.cwd(), '__tests__', 'e2e', 'real-data-integration.test.js');
...
const registry = new PluginRegistry();`
          ...
          if (content.includes('registry.get') && !content.includes('new PluginRegistry')) {
            content = content.replace(
              /registry\.get/g,
    ...
    action: () => {
      ...
      const findRerankerTests = (dir) => {
        ...
        files.forEach(file => {
          ...
          } else if (file.name.includes('reranker') && file.name.endsWith('.test.js')) {
  ...
  
  {
    ...
    action: () => {
      ...
      const metricsTestPath = path.join(process.cwd(), '__tests__', 'unit', 'observability', 'metrics.test.js');
  ...
  afterEach(() => {
    ...
    delete process.env.TEST_MODE;
          ...
          console.log(`✅ Environment variable test fixed: ${path.basename(testFile)}`);
        }
      });
    }
  },
    ...
    action: () => {
      ...
      timeoutProneTests.forEach(testFile => {
        ...
        if (fs.existsSync(testPath)) {
          ...
          fs.writeFileSync(testPath, content);
          console.log(`✅ Timeout handling improved: ${path.basename(testFile)}`);
        }
      });
    }
  }
...
async function executeTargetedFixes() {
  ...
  try {
    ...
    console.log('⚠️ Some tests still failing - progress made, continue stabilization');
  }
