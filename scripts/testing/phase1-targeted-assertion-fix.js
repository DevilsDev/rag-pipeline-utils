  ...
  fixDoctorCommandTest() {
    ...
    try {
      ...
      content = content.replace(
        /\s*expect\(true\)\.toBe\(true\);\s*\/\/\s*(?:CLI test assertion added|Added assertion)[^;]*;?/g,
        "",
      );
      content = content.replace(
      ...
      const testCases = [
        {
          pattern:
            /it\('should detect missing configuration file', async \(\) => \{[^}]*const issues = await doctor\.checkConfiguration\(\);[^}]*\}\);/s,
          replacement: (match) => {
    ...
    try {
      ...
      content = content.replace(testPattern, (match, testName, testBody) => {
        ...
        if (category === "Performance") {
          ...
          if (
            testBody.includes("process.version") ||
            testName.includes("node")
          ) {
            newTestBody +=
...
## Fixed Files
${this.fixedFiles
  .map((file) => `- \`${file.path}\` (${file.category}): ${file.fixes} fixes`)
  .join("\n")}
