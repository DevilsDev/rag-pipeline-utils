...
class TestAuditor {
    ...
    this.testFiles.forEach(testFile => {
      try {
        ...
        const fileInfo = {
          ...
          suites: describeMatches.map(match => match.replace(/describe\(['"`]([^'"`]+)['"`]/, '$1')),
          tests: testMatches.map(match => match.replace(/it\(['"`]([^'"`]+)['"`]/, '$1')),
          ...
          hasMocks: content.includes('jest.mock') || content.includes('mockReturnValue'),
  ...
  extractImports(content) {
    ...
    importMatches.forEach(match => {
      const module = match.replace(/import\s+.*\s+from\s+['"`]([^'"`]+)['"`]/, '$1');
  ...
  async runExecutionAnalysis() {
    ...
    try {
      ...
      const testOutput = execSync('npm run test:simple -- --passWithNoTests --maxWorkers=1', {
        cwd: process.cwd(),
        encoding: 'utf8',
        timeout: 60000,
        stdio: 'pipe'
      ...
      console.log('⚠️ Test execution timed out or failed - using static analysis');
      this.coverage.executionStatus = 'TIMEOUT_OR_ERROR';
      this.coverage.message = 'Could not complete full test execution within timeout';
    }
  }
    ...
    lines.forEach(line => {
      if (line.includes('Tests:') && line.includes('passed')) {
        ...
        if (match) {
          ...
          this.coverage.failedTests = this.coverage.totalTests - this.coverage.passedTests;
          this.coverage.passRate = Math.round((this.coverage.passedTests / this.coverage.totalTests) * 100);
        }
      }
      ...
      if (line.includes('Test Suites:') && line.includes('passed')) {
        ...
        if (match) {
          ...
          this.coverage.failedSuites = this.coverage.totalSuites - this.coverage.passedSuites;
          this.coverage.suitePassRate = Math.round((this.coverage.passedSuites / this.coverage.totalSuites) * 100);
        }
      }
    });
  }
  ...
  generateRecommendations() {
    ...
    if (this.coverage.passRate < 90) {
      this.recommendations.push({
        ...
        recommendation: 'Focus on fixing failing tests to achieve 90%+ pass rate'
      });
    }
    ...
    if (categoryCounts < 5) {
      this.recommendations.push({
        ...
        recommendation: 'Consider adding more test categories (unit, integration, e2e, performance, security)'
      });
    }
    
    // Issue-based recommendations
    const filesWithIssues = this.testSuites.filter(suite => suite.issues.length > 0);
    ...
    const filesWithoutSetup = this.testSuites.filter(suite => !suite.hasSetup && !suite.hasTeardown);
    if (filesWithoutSetup.length > this.testSuites.length * 0.5) {
      this.recommendations.push({
        category: 'Test Structure',
        priority: 'Low',
        issue: 'Many test files lack setup/teardown hooks',
        recommendation: 'Consider adding beforeEach/afterEach hooks for better test isolation'
      });
    }
  }
  ...
  generateReport() {
    ...
    const report = {
      ...
      categories: Object.keys(this.categories).map(cat => ({
        ...
        testCount: this.categories[cat].reduce((sum, file) => sum + file.tests.length, 0)
      })),
      ...
      detailedAnalysis: {
        filesWithIssues: this.testSuites.filter(s => s.issues.length > 0).length,
        ...
        averageTestsPerFile: Math.round(this.testCases.length / this.testSuites.length)
      }
    };
...
${report.testFiles.filter(f => f.issueCount > 0).map(file => 
