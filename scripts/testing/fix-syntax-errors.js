  ...
  fixDoctorCommandTest() {
    ...
    try {
      ...
      content = content.replace(
        /categories: \['config', 'plugins'\]\s*expect\(true\)\.toBe\(true\);\s*\/\/[^}]*}/g,
        "categories: ['config', 'plugins']\n      });",
      );

      // Fix broken function calls with malformed syntax
      content = content.replace(
        /mockResolvedValue\(JSON\.stringify\(invalidConfig\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^)]*\)/g,
        "mockResolvedValue(JSON.stringify(invalidConfig))",
      );

      // Fix broken async function calls
      content = content.replace(
        /const issues = await doctor\.checkConfiguration\(\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^;]*;/g,
        "const issues = await doctor.checkConfiguration();",
      );
      content = content.replace(
        /const issues = await doctor\.checkPlugins\(\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^;]*;/g,
        "const issues = await doctor.checkPlugins();",
      );
      content = content.replace(
        /const issues = await doctor\.checkSecurity\(\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^;]*;/g,
        "const issues = await doctor.checkSecurity();",
      );
      content = content.replace(
        /const issues = await doctor\.checkEnvironment\(\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^;]*;/g,
        "const issues = await doctor.checkEnvironment();",
      );

      // Fix broken mock chains
      content = content.replace(
        /\.mockResolvedValueOnce\(\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^.]*\./g,
      ...
      content = content.replace(
        /const result = await doctor\.autoFix\(issue\);\s*expect\(true\)\.toBe\(true\);\s*\/\/[^;]*;/g,
        "const result = await doctor.autoFix(issue);",
      );
      content = content.replace(
        /const report = doctor\.generateReport\(issues\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^;]*;/g,
        "const report = doctor.generateReport(issues);",
      );
      content = content.replace(
        /const report = await doctor\.run\(\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^;]*;/g,
        "const report = await doctor.run();",
      );

      // Fix specific broken patterns
      content = content.replace(
        /expect\(true\)\s*expect\(true\)\.toBe\(true\);\s*\/\/[^.]*\.toBe\(true\);/g,
      ...
      const testCaseFixPatterns = [
        {
          ...
          pattern:
            /it\('should initialize with custom options', \(\) => \{\s*const customDoctor = new PipelineDoctor\(\{\s*configPath: 'custom\.json',\s*verbose: true,\s*autoFix: true,\s*categories: \['config', 'plugins'\]\s*\}\);/g,
      ...
      console.log(
        `✅ Fixed ${fixCount} syntax errors in doctor-command.test.js`,
      );
    } catch (error) {
      this.errors.push(
        `Failed to fix doctor-command.test.js: ${error.message}`,
      );
