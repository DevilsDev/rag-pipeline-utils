...
const { args, logger } = setupCLI(
  ...
  {
    "--roadmap-file":
      "Path to roadmap markdown file (default: PROJECT_ROADMAP.md)",
...
async function ensureLabels(requiredLabels) {
  ...
  return await withRateLimit(
    ...
    async () => {
      ...
      const labelsToCreate = requiredLabels.filter(
        (label) => !existingLabelNames.has(label.toLowerCase()),
      );
...
async function createOrUpdateIssues(issues) {
  return await withRateLimit(
    ...
    async () => {
      ...
      logger.success(
        `Created ${created} new issues, skipped ${skipped} existing`,
      );
...
async function main() {
  try {
    ...
    logger.success(
      `🎯 Roadmap sync completed! Created: ${result.created}, Skipped: ${result.skipped}`,
    );
  } catch (error) {
