...
if (fs.existsSync(pluginHubTestPath)) {
  ...
  content = content.replace(/beforeEach\(\(\) => \{[\s\S]*?\}\);/, cleanupCode);
...
try {
  ...
  if (testMatch && suiteMatch) {
    ...
    // eslint-disable-line no-console
    console.log(
      `Tests: ${testsPassed}/${testsTotal} passed (${Math.round((testsPassed / testsTotal) * 100)}%)`,
    );
    // eslint-disable-line no-console
    console.log(
      `Suites: ${suitesPassed}/${suitesTotal} passed (${Math.round((suitesPassed / suitesTotal) * 100)}%)`,
    );
    // eslint-disable-line no-console
