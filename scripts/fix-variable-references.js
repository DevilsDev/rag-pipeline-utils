...
const fixes = [
  ...
  },
  {
    ...
    fixes: [{ from: "_options", to: "_options", lines: [19, 20] }],
  },
];
...
if (fs.existsSync(debugFix.file)) {
  ...
  if (
    lines[debugFix.line - 1] &&
    lines[debugFix.line - 1].includes(debugFix.from)
  ) {
    lines[debugFix.line - 1] = lines[debugFix.line - 1].replace(
      debugFix.from,
      debugFix.to,
    );
    fs.writeFileSync(debugFix.file, lines.join("\n"));
    console.log(
      `  ✅ Fixed line ${debugFix.line}: ${debugFix.from} → ${debugFix.to}`,
    );
    // eslint-disable-line no-console
