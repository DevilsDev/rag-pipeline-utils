...
class UndefinedVariableFixer {
  ...
  async run() {
    ...
    try {
      ...
      console.log(
        `📊 Initial undefined variable errors: ${this.report.initialErrors}`,
      );
      // eslint-disable-line no-console
      ...
      console.log(
        `✅ Fixed ${this.report.fixedErrors} undefined variable errors`,
      );
      // eslint-disable-line no-console
      ...
      console.error(
        "❌ Error during undefined variable fixing:",
        error.message,
      );
      // eslint-disable-line no-console
      process.exit(1);
  ...
  async fixCommonUndefinedVariables() {
    ...
    console.log(
      `  Found undefined variables: ${Array.from(undefinedVars).join(", ")}`,
    );
    // eslint-disable-line no-console
