...
class Batch1NoUndefFixer {
  ...
  async run() {
    ...
    try {
      ...
      console.log(`✅ Fixed ${this.report.beforeCount - this.report.afterCount} no-undef errors`);
      ...
      // Common undefined variables with clear fixes
      {
        ...
        patterns: [
          { 
            find: /\b_config\b(?!\s*[=:])/g, 
            replace: 'config',
            condition: (content) => content.includes('function') && content.includes('config')
          },
          {
            find: /\buserWorkloads\b/g,
            replace: '_userWorkloads',
            condition: (content) => content.includes('_userWorkloads') || content.includes('userWorkloads')
          },
          {
        ...
        patterns: [
          {
            ...
            condition: (content) => !content.includes("require('@jest/globals')")
          },
          {
            find: /\bdescribe\b/g,
            replace: 'describe',
            addImport: "const { describe, it, beforeEach, afterEach } = require('@jest/globals');",
            condition: (content) => !content.includes("require('@jest/globals')")
          }
        ]
      }
  ...
  async fixParameterMismatches() {
    ...
    for (const [filePath, undefinedVars] of fileErrors) {
      ...
      if (modified) {
        ...
        this.fixedFiles.push(filePath);
        this.report.filesChanged.push(path.relative(process.cwd(), filePath));
        console.log(`  ✅ Fixed parameter mismatches in ${path.relative(process.cwd(), filePath)}`);
      }
    }
  }
  ...
  writeBatchReport() {
    ...
    const reportPath = path.join('ci-reports', `eslint-fixes-${this.timestamp}.json`);
    const report = {
      batch: 1,
