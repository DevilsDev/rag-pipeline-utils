...
class FinalESLintResolver {
  ...
  async run() {
    ...
    try {
      ...
      console.log(`✅ Fixed ${this.report.fixedErrors} errors, ${this.report.finalErrors} remaining`); // eslint-disable-line no-console
      
      if (this.report.finalErrors === 0) {
        console.log('🎉 All ESLint errors resolved! Commits should now be unblocked.'); // eslint-disable-line no-console
      }
