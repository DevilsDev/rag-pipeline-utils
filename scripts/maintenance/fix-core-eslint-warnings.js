...
class CoreESLintFixer {
  ...
  async run() {
    ...
    try {
      ...
      console.log(
        `📊 Found ${this.parseErrorCount(coreErrors)} ESLint problems in core files`,
      );
      // eslint-disable-line no-console
      ...
      console.log(
        `✅ Core files ESLint status: ${remainingCount} problems remaining`,
      );
      // eslint-disable-line no-console

      if (remainingCount === 0) {
        console.log(
          "🎉 All core ESLint issues resolved! Commits should be unblocked.",
        );
        // eslint-disable-line no-console
      }
    } catch (error) {
