...
class Batch2SignatureUsageFixer {
  ...
  async run() {
    ...
    try {
      ...
      console.log(`✅ Fixed ${this.report.beforeCount - this.report.afterCount} errors`);
  ...
  getESLintErrorCount() {
    try {
      ...
      const errorLines = output.split('\n').filter(line => 
        line.includes('Error') && (line.includes('no-undef') || line.includes('no-unused-vars'))
      );
    ...
    for (const [filePath, errors] of signatureErrors) {
      ...
      for (const error of errors) {
        ...
        if (fix) {
          ...
          modified = true;
          console.log(`  ✅ Fixed ${error.variable} in ${path.relative(process.cwd(), filePath)}`);
        }
      }
  ...
  parseSignatureErrors(eslintOutput) {
    ...
    for (const line of lines) {
      ...
      const undefMatch = line.match(/^([^:]+):.*'([^']+)' is not defined.*no-undef/);
      if (undefMatch) {
      ...
      const unusedMatch = line.match(/^([^:]+):.*'([^']+)' is defined but never used.*no-unused-vars/);
      if (unusedMatch) {
  ...
  fixUnusedVariable(content, varName) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (declRegex.test(line)) {
        lines[i] = line.replace(new RegExp(`\\b${varName}\\b`, 'g'), `_${varName}`);
        modified = true;
        break;
      }
  ...
  writeBatchReport() {
    ...
    const reportPath = path.join('ci-reports', `eslint-fixes-batch2-${this.timestamp}.json`);
    const report = {
      batch: 2,
