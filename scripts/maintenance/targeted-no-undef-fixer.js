...
class TargetedNoUndefFixer {
  ...
  async run() {
    ...
    console.log(
      `🔍 Found ${undefinedVars.size} files with undefined variables`,
    );
    ...
    console.log(
      `✅ Fixed ${fixed} no-undef errors (${beforeCount} → ${afterCount})`,
    );
  ...
  async fixFileUndefinedVars(filePath, variables) {
    ...
    for (const varName of variables) {
      ...
      if (content.includes(underscoreVar)) {
        modified =
          this.fixParameterMismatch(content, varName, underscoreVar) ||
          modified;
      }
    }
  ...
  fixParameterMismatch(content, varName, underscoreVar) {
    ...
    const functionRegex = new RegExp(
      `function\\s+\\w+\\s*\\([^)]*\\b${underscoreVar}\\b[^)]*\\)`,
      "g",
    );
  ...
  addMissingImports(content, variables) {
    ...
    for (const varName of variables) {
      if (
        commonModules[varName] &&
        !content.includes(`require('${varName}')`)
      ) {
        return true;
      }
    }
