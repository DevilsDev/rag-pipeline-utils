  ...
  async fixDocsFile() {
    ...
    try {
      ...
      content = content.replace(
        /\b(error|result|_options|file|category|examples)\b(?=\s*[,;)])/g,
        (match, varName) => {
  ...
  async verifyFixes() {
    ...
    try {
      const result = execSync(
        "npx eslint src/cli/commands/docs.js --format=compact",
        {
          encoding: "utf8",
      ...
      if (error.stdout) {
        ...
        console.log(
          `⚠️  Remaining issues: ${errorCount} errors, ${warningCount} warnings`,
        );
        // eslint-disable-line no-console
