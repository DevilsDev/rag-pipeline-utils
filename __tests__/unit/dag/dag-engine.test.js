...
describe('DAG', () => {
  ...
  describe('connect', () => {
    ...
    it('should throw error for invalid source node', () => {
      expect(() => dag.connect('nonexistent', 'B')).toThrow('Invalid edge: nonexistent → B');
    });

    it('should throw error for invalid target node', () => {
      expect(() => dag.connect('A', 'nonexistent')).toThrow('Invalid edge: A → nonexistent');
    });

    it('should throw error for both invalid nodes', () => {
      expect(() => dag.connect('invalid1', 'invalid2')).toThrow('Invalid edge: invalid1 → invalid2');
    });
  });

  describe('validate', () => {
    it('should throw error for empty DAG', () => {
      expect(() => dag.validate()).toThrow('DAG is empty - no nodes to execute');
    });
    ...
    it('should detect simple cycle', () => {
      ...
      expect(() => dag.validate()).toThrow('DAG validation failed: DAG topological sort failed: Cycle detected involving node: A -> B');
    });
  ...
  describe('topoSort', () => {
    ...
    it('should detect two-node cycle', () => {
      ...
      expect(() => dag.topoSort()).toThrow('Cycle detected involving node: A -> B');
    });
    ...
    it('should detect three-node cycle', () => {
      ...
      expect(() => dag.topoSort()).toThrow('Cycle detected involving node: A -> B -> C');
    });
    ...
    it('should provide detailed cycle path information', () => {
      ...
      expect(() => dag.topoSort()).toThrow('Cycle detected involving node: middle1 -> middle2 -> end');
    });
  });
  ...
  describe('execute', () => {
    ...
    it('should throw error for empty DAG', async () => {
      await expect(dag.execute('seed')).rejects.toThrow('DAG execution failed: DAG is empty - no nodes to execute');
    });
    ...
    it('should throw error for cyclic DAG', async () => {
      ...
      await expect(dag.execute('seed')).rejects.toThrow('DAG execution failed: DAG validation failed');
    });
    ...
    it('should handle node execution failure with dependents', async () => {
      ...
      await expect(dag.execute('seed')).rejects.toThrow('Node B execution failed: B failed. This affects downstream nodes: C');
    });
    ...
    it('should continue execution when non-critical node fails', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Non-critical node failure: Node C execution failed: C failed');
    ...
    it('should handle input node failure propagation', async () => {
      ...
      await expect(dag.execute('seed')).rejects.toThrow('Node A execution failed: A failed. This affects downstream nodes: B');
    });
    ...
    it('should throw error when no sink nodes exist', async () => {
      ...
      await expect(dag.execute('seed')).rejects.toThrow('DAG has no sink nodes - no final output available');
    });
  ...
  describe('error handling edge cases', () => {
    ...
    it('should provide meaningful error messages for complex cycles', async () => {
      ...
      await expect(dag.execute('seed')).rejects.toThrow('Cycle detected involving node: branch1 -> merge -> feedback');
    });
  });
});
