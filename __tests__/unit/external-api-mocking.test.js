...
describe('External API Mocking', () => {
  ...
  describe('MockChroma', () => {
    ...
    beforeEach(() => {
      ...
      collection = mockChroma.getOrCreateCollection({ name: 'test-collection' });
    });
    ...
    test('should query collection with distance scores', async () => {
      ...
      const queryEmbeddings = [Array.from({ length: 384 }, () => Math.random())];
  ...
  describe('Integration with Jest Mocks', () => {
    test('should integrate with Jest mock functions', () => {
      ...
      expect(jest.isMockFunction(mockOpenAI.chat.completions.create)).toBe(true);
    });
