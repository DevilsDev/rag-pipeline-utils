...
describe('PipelineEventLogger', () => {
  ...
  describe('logEvent', () => {
    it('should log basic event', () => {
      logger.logEvent(EventTypes.PLUGIN_START, EventSeverity.INFO, { test: 'data' }, 'Test message');
    ...
    it('should include stack trace for errors when enabled', () => {
      ...
      logger.logEvent(EventTypes.PLUGIN_ERROR, EventSeverity.ERROR, {}, 'Error message');
    ...
    it('should maintain event history limit', () => {
      ...
      // Add 5 events
      for (let i = 0; i < 5; i++) {
        logger.logEvent(EventTypes.PLUGIN_START, EventSeverity.INFO, { index: i }, `Message ${i}`);
      }
  ...
  describe('plugin logging methods', () => {
    it('should log plugin start', () => {
      logger.logPluginStart('embedder', 'openai', ['chunk1', 'chunk2'], { batchSize: 2 });
    ...
    it('should log plugin end', () => {
      logger.logPluginEnd('embedder', 'openai', 150, { vectorCount: 2 }, { batchSize: 2 });
  ...
  describe('stage logging methods', () => {
    ...
    it('should log stage end', () => {
      logger.logStageEnd('query', 1200, { promptLength: 50, responseLength: 200 });
  ...
  describe('performance and memory logging', () => {
    it('should log performance metrics', () => {
      logger.logPerformanceMetric('embedding_duration', 150, 'ms', { plugin: 'openai' });
  ...
  describe('event history and filtering', () => {
    ...
    it('should filter by event type', () => {
      const pluginEvents = logger.getEventHistory({ eventType: EventTypes.PLUGIN_START });
      expect(pluginEvents).toHaveLength(1);
      expect(pluginEvents[0].eventType).toBe(EventTypes.PLUGIN_START);
    });

    it('should filter by severity', () => {
      const errorEvents = logger.getEventHistory({ severity: EventSeverity.ERROR });
    ...
    it('should filter by time range', () => {
      ...
      const recentEvents = logger.getEventHistory({ since: oneMinuteAgo.toISOString() });
      expect(recentEvents).toHaveLength(4); // All events should be recent
      
      const futureEvents = logger.getEventHistory({ since: new Date(now.getTime() + 60000).toISOString() });
  ...
  describe('export and session management', () => {
    ...
    it('should export filtered events', () => {
      const exported = logger.exportEvents({ eventType: EventTypes.PLUGIN_START });
      const data = JSON.parse(exported);
