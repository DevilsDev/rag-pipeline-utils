...
describe('Span', () => {
  ...
  describe('recordException', () => {
    it('should record error with details', () => {
      ...
      expect(event.attributes['exception.stacktrace']).toBe('Error: Test error\n    at test');
    });
...
describe('PipelineTracer', () => {
  ...
  describe('tracePlugin', () => {
    it('should trace successful plugin execution', async () => {
      ...
      const result = await pipelineTracer.tracePlugin('embedder', 'openai', mockPlugin, input);
    ...
    it('should trace plugin execution errors', async () => {
      ...
      await expect(pipelineTracer.tracePlugin('llm', 'gpt-4', mockPlugin, 'input'))
    ...
    it('should handle plugin with context', async () => {
      ...
      await pipelineTracer.tracePlugin('embedder', 'openai', mockPlugin, 'input', context);
  ...
  describe('traceStage', () => {
    it('should trace pipeline stage execution', async () => {
      ...
      const result = await pipelineTracer.traceStage('ingest', mockStage, context);
    ...
    it('should trace stage execution errors', async () => {
      ...
      await expect(pipelineTracer.traceStage('query', mockStage))
  ...
  describe('getTraceStats', () => {
    it('should return trace statistics', async () => {
      // Create some traces
      const mockPlugin = jest.fn().mockResolvedValue('result');
      await pipelineTracer.tracePlugin('embedder', 'openai', mockPlugin, 'input');
