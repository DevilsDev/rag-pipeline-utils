...
describe("Reranker Plugin", () => {
  ...
  describe("rerank() method", () => {
    ...
    it("should preserve original document properties", async () => {
      ...
      result.forEach((doc, index) => {
        ...
        expect(doc.originalIndex).toBe(
          sampleDocuments.findIndex((d) => d.id === doc.id),
        );
      });
    });
    ...
    it("should simulate realistic scoring based on content", async () => {
      ...
      const mlDocs = result.filter((d) =>
        d.content.includes("machine learning"),
      );
      const nonMlDocs = result.filter(
        (d) => !d.content.includes("machine learning"),
      );

      if (mlDocs.length > 0 && nonMlDocs.length > 0) {
        expect(mlDocs[0].relevanceScore).toBeGreaterThan(
          nonMlDocs[0].relevanceScore,
        );
      }
    });
    ...
    it("should give higher scores to longer content", async () => {
      const docs = [
        { id: "1", content: "Short text" },
        {
          id: "2",
          content:
            "This is a much longer piece of content that should receive a higher relevance score due to its length and comprehensive coverage of the topic at hand.",
        },
      ];
    ...
    it("should balance content length with query relevance", async () => {
      const docs = [
        ...
        },
        {
          id: "2",
          content:
            "This is a very long document about cooking and food preparation that has nothing to do with the query but is much longer than the first document and contains many words about culinary arts and kitchen techniques.",
        },
      ];
  ...
  describe("edge cases", () => {
    ...
    it("should handle very long queries", async () => {
      const longQuery =
        "machine learning artificial intelligence deep learning neural networks natural language processing computer vision reinforcement learning supervised learning unsupervised learning".repeat(
          10,
        );
      const docs = [
