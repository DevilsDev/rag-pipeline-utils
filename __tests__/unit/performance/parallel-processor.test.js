...
describe("ParallelEmbedder", () => {
  ...
  describe("embedBatch", () => {
    ...
    it("should handle empty chunks array", async () => {
      await expect(parallelEmbedder.embedBatch([])).rejects.toThrow(
        "Invalid chunks provided",
      );
    });

    it("should handle invalid chunks input", async () => {
      await expect(parallelEmbedder.embedBatch(null)).rejects.toThrow(
        "Invalid chunks provided",
      );
      await expect(parallelEmbedder.embedBatch("not-array")).rejects.toThrow(
        "Invalid chunks provided",
      );
    });
      ...
      mockRetriever.retrieve
        ...
        .mockImplementationOnce(
          () =>
            new Promise((resolve) => {
              jest.advanceTimersByTime(10);
              resolve([{ text: "result2" }]);
            }),
        );

      const results = await parallelRetriever.retrieveBatch(queryVectors);

      expect(results).toEqual([[{ text: "result1" }], [{ text: "result2" }]]);
    });
  });
});
