...
describe('PerformanceTimer', () => {
  ...
  describe('timing operations', () => {
    ...
    it('should throw error when ending non-existent timer', () => {
      expect(() => timer.end('nonexistent')).toThrow('Timer \'nonexistent\' not found');
    });
...
describe('PipelineBenchmark', () => {
  ...
  describe('benchmarkIngest', () => {
    beforeEach(() => {
      ...
      mockPipeline.embedderInstance.embed.mockResolvedValue([[1, 2], [3, 4]]);
  ...
  describe('benchmarkQuery', () => {
    beforeEach(() => {
      mockPipeline.embedderInstance.embedQuery = jest.fn().mockResolvedValue([1, 2, 3]);
    ...
    it('should handle query operation failure', async () => {
      mockPipeline.llmInstance.generate.mockRejectedValue(new Error('LLM failed'));
