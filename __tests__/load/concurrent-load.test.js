...
// Minimal pipeline stubs so tests can exercise concurrency paths quickly
const pipeline = {
...
const resilientPipeline = {
  async run(args) {
    // emulate occasional transient error if the test harness wants to retry
    if (Math.random() < 0.02) throw new Error("Transient failure");
    ...
    it("should handle mixed workload patterns", async () => {
      const workloadPatterns = [
        ...
        { type: "streaming", weight: 0.2, options: { stream: true, topK: 3 } },
        {
          type: "complex",
          weight: 0.2,
          options: { topK: 20, useReranker: true },
        },
      ];
      ...
      const memoryStressTest = PerformanceHelper.monitorMemoryUsage(
        async () => {
          ...
          for (let i = 0; i < iterations; i++) {
            ...
            const concurrentOps = [];
            for (let j = 0; j < 10; j++) {
              const queryVector = TestDataGenerator.generateVectors(1, 1536)[0]
                .values;
              concurrentOps.push(
                pipeline.run({
      ...
      const resourceExhaustionTest = async () => {
        ...
        for (let i = 0; i < 50; i++) {
          const operation = async () => {
            ...
            for (let j = 0; j < 5; j++) {
              await mockReranker.rerank(`Heavy query ${i}-${j}`, largeDataset, {
                topK: 100,
              });
            }
  ...
  describe("scalability testing", () => {
    it("should scale linearly with data size", async () => {
      ...
      for (const size of dataSizes) {
        ...
        const performance = await PerformanceHelper.measureExecutionTime(
          async () => {
            await mockRetriever.store(vectors);

            // Perform standard operations
            const queryVector = TestDataGenerator.generateVectors(1)[0].values;
            const retrievalResult = await mockRetriever.retrieve(queryVector, {
              topK: 50,
            });
    ...
    it("should handle increasing concurrent users gracefully", async () => {
      ...
      const lowConcurrency = concurrencyResults.find(
        (r) => r.concurrency === 10,
      );
      const highConcurrency = concurrencyResults.find(
        (r) => r.concurrency === 100,
      );
