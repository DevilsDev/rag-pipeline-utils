  ...
  static createMockLLM(options = {}) {
    ...
    return {
      async generate(prompt, opts = {}) {
        ...
        if (opts.stream) {
          return this.generateStream(prompt, { shouldFail, failAfterTokens, tokenDelay });
        }
      ...
      async *generateStream(prompt, streamOptions = {}) {
        ...
        for (let i = 0; i < tokens.length; i++) {
          if (streamOptions.shouldFail && i >= streamOptions.failAfterTokens) {
            throw new Error('Mock streaming failure');
          }
          
          await new Promise(resolve => setTimeout(resolve, streamOptions.tokenDelay));
          yield {
            token: tokens[i],
            done: false,
            usage: i === tokens.length - 1 ? { completionTokens: i + 1 } : null
          };
        }
...
export class TestDataGenerator {
  ...
  static generateDocuments(count = 10) {
    const topics = ['AI', 'machine learning', 'data science', 'programming', 'technology'];
    const documents = [];
