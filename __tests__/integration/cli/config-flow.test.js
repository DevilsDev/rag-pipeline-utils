...
describe("CLI integration with .ragrc.json config fallback", () => {
  beforeAll(() => {
    ...
    // Assert existence
    [FIXTURE_PDF, PDF_LOADER_SRC, EMBEDDER_SRC, RETRIEVER_SRC, LLM_SRC].forEach(
      (file) => {
        if (!existsSync(file))
          throw new Error(`[FATAL] Missing required fixture: ${file}`);
      },
    );
    ...
    copyFileSync(
      PDF_LOADER_SRC,
      join(TEMP_TEST_DIR, "src/mocks/pdf-loader.js"),
    );
    copyFileSync(
      EMBEDDER_SRC,
      join(TEMP_TEST_DIR, "src/mocks/openai-embedder.js"),
    );
    copyFileSync(
      RETRIEVER_SRC,
      join(TEMP_TEST_DIR, "src/mocks/pinecone-retriever.js"),
    );
