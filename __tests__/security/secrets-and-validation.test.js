...
describe("Security Scanning and Validation Tests", () => {
  ...
  describe("Secrets Scanning", () => {
    it("should detect hardcoded API keys and tokens", async () => {
      ...
      expect(
        testFiles.apiKeys.length +
          testFiles.passwords.length +
          testFiles.tokens.length,
      ).toBeLessThan(5);

      console.log(
        `🔍 Secrets scan: ${testFiles.totalFiles} files, ${testFiles.apiKeys.length + testFiles.passwords.length + testFiles.tokens.length} secrets found`,
      );
    });
    ...
    it("should validate environment variable usage", async () => {
      ...
      console.log(
        `🔐 Environment variables: ${envUsage.properEnvUsage} proper usage, ${envUsage.hardcodedValues.length} hardcoded values`,
      );
    });
  });
  ...
  describe("Plugin Path Validation", () => {
    it("should validate plugin file paths", async () => {
      ...
      console.log(
        `📁 Path validation: ${pathValidation.validatedPaths} validated, ${pathValidation.unsafePaths.length} unsafe paths`,
      );
    });
    ...
    it("should prevent unauthorized file system access", async () => {
      ...
      console.log(
        `🛡️ File system security: ${testCases.length} attack vectors blocked`,
      );
    });
  });
  ...
  describe("Input Validation and Sanitization", () => {
    it("should prevent injection attacks", async () => {
      ...
      console.log(
        `🚫 Injection attacks: ${injectionTests.length} attack patterns blocked`,
      );
    });
    ...
    it("should sanitize user inputs properly", async () => {
      ...
      console.log(
        `🧹 Input sanitization: ${testInputs.length} test cases validated`,
      );
    });
  });
  ...
  describe("Security Compliance", () => {
    it("should enforce security policies", async () => {
      ...
      console.log(
        `📋 Security policies: ${Object.keys(policies).length} policies enforced`,
      );
    });
    ...
    it("should validate plugin security contracts", async () => {
      ...
      console.log(
        `🔒 Plugin security: ${securityContracts.validated} contracts validated, ${securityContracts.failed} failed`,
      );
    });
  });
      ...
      validateInput(input) {
        ...
        for (const [key, value] of Object.entries(input)) {
          if (typeof value === "string") {
            ...
            if (
              value.toLowerCase().includes("drop table") ||
  ...
  async function generateSecurityReports() {
    ...
    const jsonReport = {
      ...
      summary: {
        ...
        highSeverityIssues: securityResults.filter((r) => r.severity === "HIGH")
          .length,
        mediumSeverityIssues: securityResults.filter(
          (r) => r.severity === "MEDIUM",
        ).length,
        lowSeverityIssues: securityResults.filter((r) => r.severity === "LOW")
          .length,
      },
