...
{
  "lint-staged": {
    "**/*.{js,ts}": ["eslint --fix --max-warnings=Infinity --quiet"],
...
_This configuration prioritizes developer productivity while maintaining code quality. Rules are designed to catch real issues without creating friction in the development process._
